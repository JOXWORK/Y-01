...
from core.tasks.moderation_rules.get_human_readable_moderation_rules_db import get_human_readable_moderation_rules_db_task
...
async def create_request(user_id: int, rules_schema: ModerationRulesSchema) -> TaskIDSchema:
...
async def get_human_readable_request(user_id: int) -> ModerationRulesSchema:
    task = await get_human_readable_moderation_rules_db_task.kiq(user_id)
